...
export function extractMetadata(html: string, pageUrl: string): NormalizedMetadata {
  const titleFromMeta = firstDefined(
    getMetaContents(html, ['og:title', 'twitter:title']),
    getTitleTag(html),
  )
  const description = firstDefined(
    getMetaContents(html, ['og:description', 'twitter:description', 'description']),
    null,
  )
...
function pickBestFavicon(html: string, pageUrl: string): string | null {
  const linkTags = html.match(/<link\b[^>]*>/gi) ?? []

...
import { fetchGithubMetadata, extractGithubLogin, isGithubProfileUrl } from '../github'
...
describe('github metadata', () => {
  it('detects GitHub profile URLs and extracts the login', () => {
    expect(isGithubProfileUrl(new URL('https://github.com/octocat'))).toBe(true)
    expect(extractGithubLogin(new URL('https://github.com/octocat/'))).toBe('octocat')
    expect(extractGithubLogin(new URL('https://github.com/octocat?tab=repositories'))).toBe('octocat')
    expect(isGithubProfileUrl(new URL('https://github.com/octocat/harune'))).toBe(false)
    expect(extractGithubLogin(new URL('https://example.com/octocat'))).toBeNull()
  })

  it('builds a 31 day contribution metadata payload from GitHub GraphQL', async () => {
    const now = new Date('2026-05-12T12:00:00.000Z')

    const githubDays = Array.from({ length: 31 }, (_, index) => {
      const day = new Date(now)
      day.setUTCDate(day.getUTCDate() - (30 - index))

...
function toProfilePageResponse(
	page: ProfilePageSummary,
): ProfilePageResponse["page"] {
	return {
		id: page.id,
		userId: page.userId,
		handle: page.handle,
...
function parseMediaBentoContent(value: unknown) {
	...
	const caption =
		value.caption === undefined
			? ""
			: (parseTrimmedString(value.caption) ?? "");
	const tempObjectKey =
		value.tempObjectKey === undefined
			? null
			: parseTrimmedString(value.tempObjectKey);
	const contentHash =
...
export function createProfileRoute(
	...
	const updatePageImageByUserId =
		dependencies.updateProfilePageImageByUserId ??
		updateProfilePageImageByUserId;
	...
	return new Hono<AppBindings>()
		...
		.patch("/image", async (c) => {
			try {
				...
				const target = parseProfileImageTarget(
					c.env.R2_PUBLIC_BASE_URL,
				...
				if (
					...
					target.imageKind !== parsed.output.imageKind
				) {
					return withNoStore(
						forbidden(
							c,
				...
				if (!object) {
					return withNoStore(
						notFound(
							...
							"profile image object not found",
						),
					);
				}

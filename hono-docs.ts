			...
			api: [
				...
				},
				{
					...
					description: [
						...
						"- `preview:` bento uploads are stored directly at a public preview object key so the later save can avoid temp-to-final copy work",
						"- File type must be image or video",
						"- File size must be 5MB or smaller",
						"- A SHA-256 hash is computed before storage write",
						"- Uploads only return media metadata; no final DB write happens here",
						"- The response is `Cache-Control: no-store`",
					].join("\n"),
					tag: ["Profile API"],
				},
